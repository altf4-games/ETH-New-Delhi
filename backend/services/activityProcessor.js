...
export async function processActivity(eventData) {
  ...
  try {
    ...
    const zoneCaptures = await checkZoneCaptures(
      h3Zones,
    ...
    try {
      ...
      console.error(
        "NFT minting failed, but activity processed successfully:",
        nftError
      );
...
async function checkZoneCaptures(h3Zones, totalPoints, userId) {
  ...
  for (const zone of h3Zones.slice(0, 3)) {
    // Check top 3 zones
...
async function mintNFTForCompletedRun(
  processedActivity,
  pointsResult,
  h3Zones
) {
  try {
    ...
    const meetsNFTCriteria = checkNFTMintingCriteria(
      processedActivity,
      pointsResult
    );
    ...
    if (!meetsNFTCriteria.shouldMint) {
      console.log(
        "Run does not meet NFT minting criteria:",
        meetsNFTCriteria.reason
      );
      return null;
    }
...
function checkNFTMintingCriteria(processedActivity, pointsResult) {
  ...
  if (runStats.distance < minDistance) {
    return {
      shouldMint: false,
      reason: `Distance ${runStats.distance}m is less than required ${minDistance}m`,
    };
  }
  ...
  if (pointsResult.totalPoints < minPoints) {
    return {
      shouldMint: false,
      reason: `Points ${pointsResult.totalPoints} is less than required ${minPoints}`,
    };
  }
...
function calculateRunStatistics(processedActivity) {
  ...
  const duration =
    timeData.length > 0 ? timeData[timeData.length - 1] - timeData[0] : 0;
...
function determinePrimaryZone(h3Zones) {
  ...
  return {
    name:
      h3Zones.length > 0 ? `Zone ${h3Zones[0].slice(0, 8)}` : "Fitness Zone",
